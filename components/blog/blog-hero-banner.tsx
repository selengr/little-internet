'use client'

import Image from 'next/image'
import { BackgroundGradientAnimation } from '@/components/ui/background-gradient-animation'
import styles from '@/components/views/banner/banner.module.css'

type BlogHeroBannerProps = {
  title: string
  subtitle: string
  bannerImage?: string
  authorImage: string
}

export function BlogHeroBanner({
  title,
  subtitle,
  bannerImage = '/images/banners/fikeus-west-2.avif',
  authorImage,
}: BlogHeroBannerProps) {
  return (
    <div className={styles['landing-main']}>
      <div className="relative mx-auto w-full max-w-[1200px]">
        <div className="relative h-[32vh] min-h-[260px] max-h-[420px] overflow-hidden rounded-none sm:rounded-[24px] shadow-[0_24px_80px_-28px_rgba(88,28,135,0.45)] ring-1 ring-black/[0.06] dark:ring-white/[0.08] max-sm:w-screen max-sm:relative max-sm:left-1/2 max-sm:-translate-x-1/2">
          <BackgroundGradientAnimation
            interactive
            containerClassName="absolute inset-0 h-full w-full"
            gradientBackgroundStart="rgb(8, 8, 14)"
            gradientBackgroundEnd="rgb(24, 14, 48)"
            firstColor="139, 92, 246"
            secondColor="245, 158, 11"
            thirdColor="34, 211, 238"
            fourthColor="236, 72, 153"
            fifthColor="74, 222, 128"
            pointerColor="196, 181, 253"
            size="88%"
            blendingValue="soft-light"
          />

          <Image
            src={bannerImage}
            alt=""
            fill
            priority
            className="pointer-events-none object-cover object-center opacity-[0.2] mix-blend-soft-light"
            sizes="(max-width: 1200px) 100vw, 1200px"
          />

          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/40 to-transparent" />
          <div className="pointer-events-none absolute inset-0 rounded-none sm:rounded-[24px] ring-1 ring-inset ring-white/10" />
        </div>

        <div
          className={`${styles['landing-div-rounded']} bg-[#f7f6f3] overflow-hidden relative z-10`}
        >
          <Image
            src={authorImage}
            alt="Author"
            width={124}
            height={124}
            className={`${styles['landing-div-rounded-home']} ${styles['landing-div-rounded-avatar']}`}
            style={{ objectFit: 'cover' }}
          />
        </div>
      </div>

      <h1 className={`${styles['landing-title']} bg-gradient-to-br from-foreground via-foreground to-foreground/70 bg-clip-text`}>
        {title}
      </h1>

      <div className="w-full flex justify-center">
        <article className={styles['landing-article']}>
          <p
            className="text-center text-[17px] sm:text-[19px] leading-[1.55] text-muted-foreground tracking-[-0.01em] max-w-xl mx-auto"
            dir="ltr"
          >
            {subtitle}
          </p>
        </article>
      </div>
    </div>
  )
}
